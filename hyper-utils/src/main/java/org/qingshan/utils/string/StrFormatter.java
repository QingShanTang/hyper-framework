...
public class StrFormatter {
        ...
        for (int argIndex = 0; argIndex < argArray.length; argIndex++) {
            ...
            if (delimIndex > 0 && strPattern.charAt(delimIndex - 1) == StrPool.C_BACKSLASH) {// 转义符
                if (delimIndex > 1 && strPattern.charAt(delimIndex - 2) == StrPool.C_BACKSLASH) {// 双转义符
                    ...
                    sbuf.append(StrUtil.obj2Str(argArray[argIndex]));
    ...
    public static String format(CharSequence template, Map<?, ?> map, boolean ignoreNull) {
        ...
        for (Map.Entry<?, ?> entry : map.entrySet()) {
            value = StrUtil.obj2Str(entry.getValue());
            if (null == value && ignoreNull) {
                continue;
            }
            template2 = StringUtils.replace(template2, "{" + entry.getKey() + "}", null == value ? "" : value);
        }
        return template2;
    }
}
